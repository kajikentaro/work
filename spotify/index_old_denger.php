...
if ($_GET['code']) {
	...
	$dot = $api->getUserPlaylists('s1pfbd43e2xtfahw4pcuyxea0', [
	]);
	...
	$test = $api->getUserPlaylists('testacounts890', [
	]);
	foreach ($test->items as $playlist) {
		$response = $api->createPlaylist([
		    'name' => $playlist->name
		]);
		$playlistTracks = $api->getPlaylistTracks($playlist->id);
 ...
 */
	$super= $api->getUserPlaylists('superbeaconhumstar', [
	]);
