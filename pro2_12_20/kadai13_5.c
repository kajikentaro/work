//2019/12/20 課題13e 19D8104026J 可児憲太郎
#include <stdio.h>
#include "gnuplot.h"
...
int main(void){
	gnuplot_init();
	FILE *input;
	input=fopen("points_ang_1000.txt","r");
	int n;
	fscanf(input,"%d",&n);
	int i;
	double x[1024],y[1024],ang[1024];
	...
	int x_position;
	for(i=0;i<n;i++){
		fscanf(input,"%lf %lf %lf",&x[i],&y[i],&ang[i]);
		if(i==0||x_min>x[i]){
			x_min=x[i];
			x_position=i;
		}
	...
	for(i=0;i<n;i++){
		...
		int ang_position;
		for(j=n-1;j>=i;j--){
			if(j==n-1||ang_min>ang[seq[j]]){
				ang_min=ang[seq[j]];
				ang_position=j;
			}
		}
		int tmp=seq[ang_position];
		seq[ang_position]=seq[i];
		seq[i]=tmp;
	}
	int start;
	for(i=0;i<n;i++){
